<?php
	...
	{		
		...
		$spec_part_ref_des = $_POST['edtRefDes'];  
		...
		if(is_numeric($spec_item_id)) //Редактирование информации об элементе спецификации в БД
		{
			...
			mysqli_query($db, "UPDATE board_spec SET ref_des = \"$spec_part_ref_des\", count = \"$spec_part_count\", part_id = \"$spec_part_id\" WHERE id = \"$spec_item_id\"") or die ("<p>Не удалось обновить информацию об элементе спецификации в БД</p>");
		}
		else //Добавление нового элемента спецификации
		{
			//Запись нового элемента спецификации в базе данных
			mysqli_query($db, "INSERT INTO board_spec (ref_des, count, part_id, board_id) VALUES (\"$spec_part_ref_des\", \"$spec_part_count\", \"$spec_part_id\", \"$board_id\")") or die ("<p>Не удалось добавить элемент спецификации</p>");
		}
		
		//Переходим на страницу редактирования платы
		header("Location: board_config.php?id=$board_id"); exit();
	}
	...
	if(isset($_POST['btnSpecItemDelete']))
	{
		...
		mysqli_query($db, "DELETE FROM board_spec WHERE id = \"$spec_item_id\"") or die ("<p>Не удалось удалить элемент спецификации</p>");
...
<html lang="en">
	<head>
		...
		<title>Элемент входящий в плату</title>
...
<?php			
	if(isset($spec_item_id))
	{
		...
		if($spec_part_user_id != $user_id)
			die("Ошибка загрузки информации о элементе входящем в плату");
	}
	
	echo "			<form id = 'frmSpecItemConfig' method = 'post' action='board_part_config.php?board_id=$board_id&spec_item_id=$spec_item_id'>	
						<div class='mb-3'>
							<h4 class = 'text-center'>Элемент входящий в плату</h4>
						</div>
						<div class='mb-3'>
	...
	echo "				</div>
						<div class = 'mb-3'>
							<label class = 'form-label' for = 'edtRefDes'>Позиционное обозначение:</label>
							<input class = 'form-control' name = 'edtRefDes' type='text' value='$spec_part_ref_des' placeholder='Позиционное обозначение элементов на плате'>
						</div>
						<div class = 'mb-3'>
							<label class = 'form-label' for = 'edtCount'>Количество:</label>
							<input class = 'form-control' name = 'edtCount' type='text' value='$spec_part_count' placeholder='Общее число элементов на плате'>
						</div>						  
						<div class = 'row justify-content-center'>
