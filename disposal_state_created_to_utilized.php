<?php
	...
	if($disposal_user_id != $user_id)
		die("Ошибка списания элементов со склада");
			
	//Исключаем возможможность повторного списания элементов со склада
	...
	for($i = 0; $i < $row_count; $i++)
	{
		...
		mysqli_query($db, "UPDATE part_list SET count = (count - \"$part_count\") WHERE id = \"$part_id\"");
	}
	
	//Изменяем статус на "Списано"
	mysqli_query($db, "UPDATE disposal_list SET status = 'UTILIZED' WHERE id = \"$id\"");
