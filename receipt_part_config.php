<?php
	...
	//Идентификатор поставки элементов в БД
	...
	{		
		...
		if(is_numeric($spec_item_id)) //Редактирование информации об элементе спецификации в БД
		{
			...
			mysqli_query($db, "UPDATE receipt_spec SET cost = \"$spec_part_cost\", count = \"$spec_part_count\", part_id = \"$spec_part_id\" WHERE id = \"$spec_item_id\"") or die ("<p>Не удалось обновить информацию об элементе спецификации в БД</p>");
		}
		else //Добавление нового элемента спецификации
		{
			//Запись нового элемента спецификации в базе данных
			mysqli_query($db, "INSERT INTO receipt_spec (cost, count, part_id, receipt_id) VALUES (\"$spec_part_cost\", \"$spec_part_count\", \"$spec_part_id\", \"$receipt_id\")") or die ("<p>Не удалось добавить элемент спецификации</p>");
		}
		
		//Переходим на страницу редактирования поставки элементов
		header("Location: receipt_config.php?id=$receipt_id"); exit();
	}
	...
	if(isset($_POST['btnSpecItemDelete']))
	{
		...
		mysqli_query($db, "DELETE FROM receipt_spec WHERE id = \"$spec_item_id\"") or die ("<p>Не удалось удалить элемент спецификации</p>");
...
<html lang="en">
	<head>
		...
		<title>Элемент входящий в поставку</title>
...
<?php			
	if(isset($spec_item_id))
	{
		...
		$query_result = mysqli_query($db,  "SELECT 
												...
												receipt_spec.part_id AS part_id,
												receipt_spec.cost AS cost,
												receipt_spec.count AS count
											...
											WHERE receipt_spec.id = $spec_item_id");

		$row = mysqli_fetch_array($query_result);
		
		$spec_part_type_id = $row['part_type_id'];
		$spec_part_id = $row['part_id'];
		$spec_part_cost = round($row['cost'], 2);
		$spec_part_count = $row['count'];
		$spec_part_user_id = $row['part_user_id'];
		
		//Проверяем принадлежность элемента входящего в поставку пользователю
		if($spec_part_user_id != $user_id)
			die("Ошибка загрузки информации об элементе входящем в поставку");
	}
	
	echo "			<form id = 'frmSpecItemConfig' method = 'post' action='receipt_part_config.php?receipt_id=$receipt_id&spec_item_id=$spec_item_id'>	
						<div class='mb-3'>
							<h4 class = 'text-center'>Элемент входящий в поставку</h4>
						</div>
						<div class='mb-3'>
	...
	echo "				</div>
						<div class = 'mb-3'>
							<label class = 'form-label' for = 'edtCost'>Цена:</label>
							<input class = 'form-control' name = 'edtCost' type='text' value='$spec_part_cost' placeholder='Цена за штуку'>
						</div>
						<div class = 'mb-3'>
							<label class = 'form-label' for = 'edtCount'>Количество:</label>
							<input class = 'form-control' name = 'edtCount' type='text' value='$spec_part_count' placeholder='Количество элементов в поставке'>
						</div>						  
						<div class = 'row justify-content-center'>
