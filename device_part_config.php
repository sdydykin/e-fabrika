<?php
	...
	{		
		...
		if(is_numeric($spec_item_id)) //Редактирование информации об элементе спецификации в БД
		{
			...
			mysqli_query($db, "UPDATE device_spec SET item_id = \"$spec_part_id\", count = \"$spec_part_count\" WHERE id = \"$spec_item_id\"") or die ("<p>Не удалось обновить информацию об элементе спецификации в БД</p>");
		}
		else //Добавление нового элемента спецификации
		{
			//Запись нового элемента спецификации в базе данных
			mysqli_query($db, "INSERT INTO device_spec (device_id, count, item_id, item_type) VALUES (\"$device_id\", \"$spec_part_count\", \"$spec_part_id\", \"part\")") or die ("<p>Не удалось добавить элемент спецификации</p>");
		}
...
<html lang="en">
	<head>
		...
		<title>Элемент входящий в устройство</title>
...
<?php			
	if(isset($spec_item_id))
	{
		...
		$spec_part_count = $row['part_count'];
		$spec_part_id = $row['part_id'];
		$spec_part_type_id = $row['part_type_id'];
		$spec_part_user_id = $row['part_user_id'];
		
		//Проверяем принадлежность элемента входящего в устройство пользователю
		if($spec_part_user_id != $user_id)
			die("Ошибка загрузки информации об элементе входящем в устройство");
	}
	
	echo "			<form id = 'frmSpecItemConfig' method = 'post' action='device_part_config.php?device_id=$device_id&spec_item_id=$spec_item_id'>	
						<div class='mb-3'>
							<h4 class = 'text-center'>Элемент входящий в устройство</h4>
						</div>
						<div class='mb-3'>
	...
	echo "				</div>						
						<div class = 'mb-3'>
							...
							<input class = 'form-control' name = 'edtCount' type='text' value='$spec_part_count' placeholder='Число элементов входящих в устройство'>
						</div>						  
						<div class = 'row justify-content-center'>
